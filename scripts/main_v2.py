...
RUNTIME_DIR = os.environ.get("RUNTIME_DIR", os.path.join(BASEDIR, "runtime"))  # kernels & db
SINGBOX_BIN = os.environ.get("SINGBOX_BIN", os.path.join(RUNTIME_DIR, "sing-box"))
...
def download_file(url: str, dest: str, timeout: int = 35, retries: int = 2):
    """下载文件到本地; 分块流式 + 原子替换 + 多镜像加速 + 实时进度"""
    if os.path.exists(dest) and os.path.getsize(dest) > 1024:
        return

    filename = os.path.basename(dest)

    # 针对 GitHub 资源生成国内高速镜像
    mirrors = []
    if "github.com" in url or "raw.githubusercontent.com" in url:
        # jsdelivr 镜像处理 (raw 文件有效)
    ...
    for mirror in mirrors:
        try:
            ...
            print(f"[*] 尝试从源 [{domain}] 下载 {filename}...")
                        ...
                        if chunk:
                            ...
                            if now - last_log >= 2.0:
                                ...
                                if total_size > 0:
                                    ...
                                    print(f"[*] {filename} 下载进度: {downloaded//(1024*1024)}MB / {total_size//(1024*1024)}MB ({pct}%)")
                                else:
                                    print(f"[*] {filename} 已下载: {downloaded//1024} KB...")

            if os.path.getsize(tmp) < 1024:
                raise RuntimeError(f"下载不完整: {os.path.getsize(tmp)} bytes")
            os.replace(tmp, dest)
            print(f"[+] {filename} 下载完成! (大小: {os.path.getsize(dest)//1024} KB)")
            return
        except Exception as e:
            last_err = e
            print(f"[!] 源 [{domain}] 连接超时或失败 ({str(e)[:50]}), 自动切换下一个源...")
    ...
    try:
        ...
        pass
    raise RuntimeError(f"所有下载镜像均失败 ({filename}): {last_err}")
...
def setup_environment():
    try:
        ...
        da.sync_all_assets(Path(RUNTIME_DIR))
    except Exception as e:
        print(f"[!] 自动同步组件异常: {e}")
        raise
